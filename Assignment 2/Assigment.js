...
(async function main() {
  ...
  var UsersCopy = await getUsers();
  console.log(Users);
  let arrays = sortGender(Users);
  console.log(arrays[0]);
  console.log(arrays[1]);
  console.log(femaleToString(arrays[1]));
  ageDate(arrays[0]);
  console.log(arrays[0]);
  console.log(sum(UsersCopy));
})();

let sum = (Users) => {
  Users.forEach((element) => {
    usersCount++;
    ages += parseInt(element.dob.age);
  });
  return [ages, usersCount];
};

function ageDate(Males) {
  dobString = "";
  Males.forEach((element) => {
    dobString = element.dob.age + ": " + element.dob.date;
    element.dob = dobString;
  });
}

function femaleToString(Females) {
  Females.forEach((female) => {
    f = "";
    f = female.name.title + " " + female.name.first + " " + female.name.last;
    femalesString.push(f);
  });
  return femalesString;
}

function sortGender(users) {
  users.forEach((element) => {
